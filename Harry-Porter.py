...
# initial funtion for callin of trackbar
def hello(x):
    # only fo refernce
    print("")
...
cv2.createTrackbar("upper_value", "bars", 255, 225, hello)
...
# capturing the initial frame for creating background
while (True):
    cv2.waitKey(1000)
    ret, init_frame = cap.read()
    # check if frame iss returned then break
    if (ret):
        break

# Start Capturing the frames for thee actual magic
while (True):
    ret, frame = cap.read()
    ...
    #mixing of frames in combination to achieve the required frame
    b=frame[:,:,0]
    g=frame[:,:,1]
    r=frame[:,:,2]
